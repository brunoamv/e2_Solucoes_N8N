#!/usr/bin/env python3
"""
Generate WF07 V9.3 - Safe Property Access
Date: 2026-04-01
Solution: Fix 'in' operator error when rawData is array (execution 18273)
Fixes: Cannot use 'in' operator to search for 'data' in [line 8]
"""
...
workflow = {
    "name": "07 - Send Email V9.3 (Safe Property Check)",
    "nodes": [
        # Node 1: Execute Workflow Trigger
        {
            ...
            "typeVersion": 1,
            "position": [250, 300],
            "notes": "V9.3: Trigger for WF05 integration"
        },

        # Node 2: Prepare Email Data (unchanged from V9.2)
        {
            "parameters": {
                "jsCode": """// Prepare Email Data - V9.3
...
console.log('📧 [Prepare Email Data V9.3] Input source:', {
    isFromWF05,
...
console.log('✅ [Prepare Email Data V9.3] Data prepared:', {
...
"""
            ...
            "typeVersion": 2,
            "position": [450, 300],
            "notes": "V9.3: Prepare data and determine template"
        },
        ...
        # Node 3: Fetch Template (HTTP Request)
        {
            ...
            "typeVersion": 4.2,
            "position": [650, 300],
            "notes": "V9.3: Fetch HTML template from nginx"
        },

        # Node 4: Render Template (SAFE PROPERTY CHECK - CRITICAL FIX)
        {
            "parameters": {
                "jsCode": """// Render Template - V9.3 (Safe Property Check)
...
if (typeof rawData === 'string') {
    ...
    console.log('✅ [V9.3] Case 1: Direct string (length:', templateHtml.length, ')');

} else if (Array.isArray(rawData)) {
    // Case 2: Direct array response (FIXED - detected BEFORE property check)
    templateHtml = rawData.join('');
    console.log('✅ [V9.3] Case 2: Direct array converted (length:', templateHtml.length, ')');
    ...
    if (rawData.data && typeof rawData.data === 'string') {
        ...
        console.log('✅ [V9.3] Case 3: Object.data as string (length:', templateHtml.length, ')');

    // Case 4: Object with 'data' property (array)
    } else if (rawData.data && Array.isArray(rawData.data)) {
        templateHtml = rawData.data.join('');
        console.log('✅ [V9.3] Case 4: Object.data as array converted (length:', templateHtml.length, ')');
    ...
    // Case 5: Object with 'data' property (object with numeric keys)
        ...
        console.log('✅ [V9.3] Case 5: Object.data with numeric keys converted (length:', templateHtml.length, ')');
    ...
    // Case 6: Object with numeric keys directly (no 'data' wrapper)
        ...
        console.log('✅ [V9.3] Case 6: Root object with numeric keys converted (length:', templateHtml.length, ')');

    // Case 7: Unknown object structure
    } else {
        console.error('❌ [V9.3] Unknown object structure:', {
    ...
    console.error('❌ [V9.3] Completely unknown format:', typeof rawData);
...
console.log('📝 [V9.3] Template conversion successful:', {
...
const renderTemplate = (html, data) => {
    ...
    console.log('🔄 [Render V9.3] Starting template rendering');
...
console.log('✅ [Render V9.3] Template rendered successfully:', {
...
"""
            ...
            "typeVersion": 2,
            "position": [850, 300],
            "notes": "V9.3: Safe property check (fixes exec 18273)"
        },

        # Nodes 5-7: Unchanged from V9.2
        {
            "parameters": {
            ...
            "notes": "V9.3: Send email via SMTP"
        },
        {
            "parameters": {
            ...
            "notes": "V9.3: Log email to database"
        },
        {
            "parameters": {
...
"""
            ...
            "typeVersion": 2,
            "position": [1450, 300],
            "notes": "V9.3: Return success response"
        }
    ],
    ...
    "tags": [
        ...
        {"name": "v9.3", "createdAt": "2026-04-01T00:00:00.000000"},
        {"name": "safe-property-check", "createdAt": "2026-04-01T00:00:00.000000"}
    ],
    "triggerCount": 1,
...
output_file = "/home/bruno/Desktop/Programas/E2_Solucoes/e2-solucoes-bot/n8n/workflows/07_send_email_v9.3_safe_property_check.json"

with open(output_file, 'w', encoding='utf-8') as f:
    json.dump(workflow, f, indent=2, ensure_ascii=False)

print(f"✅ WF07 V9.3 generated: {output_file}")
print(f"   - 7 nodes (Safe property check for arrays)")
print(f"   - Fixes: execution 18273 ('in' operator error)")
print(f"   - Size: {len(json.dumps(workflow))} bytes")
print("")
print("🎯 V9.3 Critical Fix:")
print("   ❌ V9.2: 'data' in rawData (fails on arrays)")
print("   ✅ V9.3: Safe check before 'in' operator")
print("")
print("🔧 Next Steps:")
print("   1. Import: http://localhost:5678 → Import from File")
print("   2. Select: 07_send_email_v9.3_safe_property_check.json")
print("   3. Test with execution 18273 data")
print("   4. Verify: No 'in' operator errors")
