#!/usr/bin/env python3
"""
Fix duplicate WhatsApp messages issue in workflow 01.
...
def fix_duplicate_messages(input_file, output_file):
    """
    Fix the Save Message node in workflow 01 to handle duplicate WhatsApp message IDs.
    """
    ...
    # Find and fix the Save Message node
    for node in workflow.get('nodes', []):
        if node.get('name') == 'Save Message' and node.get('type') == 'n8n-nodes-base.postgres':
            print(f"Found Save Message node, fixing duplicate handling...")
            ...
            # Note: Using expression syntax since the original uses it
            new_query = "=INSERT INTO messages (conversation_id, direction, content, message_type, media_url, whatsapp_message_id) VALUES (null, 'inbound', '{{ $json.content.replace(/'/g, \"''\") }}', '{{ $json.message_type }}', {{ $json.media_url ? \"'\" + $json.media_url.replace(/'/g, \"''\") + \"'\" : \"null\" }}, '{{ $json.message_id.replace(/'/g, \"''\") }}') ON CONFLICT (whatsapp_message_id) DO UPDATE SET updated_at = CURRENT_TIMESTAMP, content = EXCLUDED.content, media_url = EXCLUDED.media_url RETURNING id, created_at, CASE WHEN xmax = 0 THEN 'inserted' ELSE 'updated' END as operation"
            ...
            modified = True
            print("✅ Save Message node fixed with ON CONFLICT handling")

    if not modified:
        print("⚠️ No Save Message node found to fix")
...
def main():
    ...
    output_file = base_dir / "n8n/workflows/01_main_whatsapp_handler_V2.5_DEDUP_FIXED.json"
    ...
    if success:
        ...
        print("2. Deactivate the old workflow 01")
        print("3. Activate the new workflow")
        print("4. Test with a WhatsApp message")

        print("\n🔍 To monitor for duplicates:")
        print("docker logs -f e2bot-n8n-dev 2>&1 | grep -E '(duplicate|conflict|inserted|updated|operation)'")
