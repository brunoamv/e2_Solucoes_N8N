#!/usr/bin/env python3
"""
V60.2 COMPLETE FLOW - Generator Script (STATE TRANSITIONS FIX)

Purpose: Generate V60.2 workflow with complete flow including intermediate states
Critical Fix: Update state transitions to include phone_confirmation and confirmation states

Changes from V60.1:
1. ✅ Fix collect_phone → transitions to collect_phone_whatsapp_confirmation
2. ✅ Fix collect_city → transitions to confirmation (with summary generation)
3. ✅ Keep all V60.1 fixes (functionCode, rich templates, state logic)
...
# ============================================================================
# V59 RICH TEMPLATES (16 TEMPLATES - SAME AS V60.1)
# ============================================================================
...
# ============================================================================
# V60 CONFIRMATION LOGIC (SAME AS V60.1)
# ============================================================================
...
# ============================================================================
# STATE TRANSITION FIXES FOR V60.2
# ============================================================================

def fix_collect_phone_transition(js_code):
    """
    Fix collect_phone state to transition to collect_phone_whatsapp_confirmation
    instead of directly to collect_email
    """
    print("🔧 Fixing collect_phone → collect_phone_whatsapp_confirmation transition")

    # Pattern to find collect_phone case and its transitions
    pattern = r"(case 'collect_phone':[\s\S]*?)(responseText = templates\.collect_email;[\s]*nextStage = 'collect_email';)"

    replacement = r"\1// V60.2 FIX: Transition to phone confirmation state\n      const formattedPhone = formatPhoneDisplay(cleanPhone);\n      responseText = templates.collect_phone_whatsapp_confirmation.replace('{{phone}}', formattedPhone);\n      nextStage = 'collect_phone_whatsapp_confirmation';"
...
def fix_collect_city_transition(js_code):
    """
    Fix collect_city state to transition to confirmation (with summary)
    instead of directly to completed
    """
    print("🔧 Fixing collect_city → confirmation transition")

    # Pattern to find collect_city case and replace completion logic with confirmation
    pattern = r"(case 'collect_city':[\s\S]*?updateData\.city = message;[\s]*)([\s\S]*?)(nextStage = 'completed';)"

    # New summary generation and confirmation logic
    replacement = r"""\1
    ...
    print("✅ Fixed collect_city transition with summary generation")
...
def main():
    print("=" * 60)
    print("V60.2 Complete Flow - Workflow Generator (STATE TRANSITIONS FIX)")
    print("=" * 60)
    print()
    ...
    v60_2_file = base_dir / "n8n" / "workflows" / "02_ai_agent_conversation_V60_2_COMPLETE_FLOW.json"
    ...
    # Step 3: Get JavaScript code from FUNCTIONCODE
    ...
    # Step 5: Fix state transitions (V60.2 CRITICAL FIXES)
    print("🔧 Applying V60.2 state transition fixes")
    js_code = fix_collect_phone_transition(js_code)
    js_code = fix_collect_city_transition(js_code)
    print()

    # Step 6: Add V60 confirmation logic
    ...
    # Step 7: Update stateNameMapping
  ...
  'menu_correcao': 'correction_menu'
    ...
    workflow['name'] = '02 - AI Agent V60.2 (Complete Flow - ALL STATES)'
    workflow['settings']['executionOrder'] = 'v1'

    if 'meta' not in workflow:
        workflow['meta'] = {}

    workflow['meta']['instanceId'] = 'v60.2-complete-flow'

    # Step 10: Write V60.2 workflow
    print("💾 Writing V60.2 workflow")
    with open(v60_2_file, 'w', encoding='utf-8') as f:
        json.dump(workflow, f, indent=2, ensure_ascii=False)

    file_size = v60_2_file.stat().st_size
    ...
    print(f"   ✅ CRITICAL FIX #1: collect_phone → collect_phone_whatsapp_confirmation")
    print(f"   ✅ CRITICAL FIX #2: collect_city → confirmation (with summary)")
    print(f"   ✅ Updated functionCode (n8n executes this)")
    print(f"   ✅ Removed jsCode (n8n ignores it)")
    print()
    print("=" * 60)
    print("✅ V60.2 WORKFLOW GENERATION COMPLETE")
    print("=" * 60)
    print()
    print("📁 Output file:", v60_2_file)
    print()
    print("🚀 Next Steps:")
    print("   1. Import V60.2 to n8n: http://localhost:5678")
    print("   2. Deactivate V60.1")
    print("   3. Activate V60.2")
    print("   4. Test complete flow:")
    print("      oi → 1 → Bruno Rosa → (62) 98765-4321")
    print("      → [PHONE CONFIRMATION] → 1 → bruno@email.com")
    print("      → Goiânia - GO → [DATA CONFIRMATION] → sim")
    print("   5. Verify all intermediate states appear")
    print()
