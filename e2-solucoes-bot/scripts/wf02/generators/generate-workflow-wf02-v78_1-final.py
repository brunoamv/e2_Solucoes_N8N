#!/usr/bin/env python3
"""
Generate WF02 V78.1.1 FINAL - Fixed Duplicate Nodes + Parallel Connections
===========================================================================

CRITICAL FIXES FROM V78 COMPLETE AND V78.1:
- V78 PROBLEM #1: Created duplicate "Update Conversation State" node
- V78 PROBLEM #2: Broke parallel connections
- V78.1 FIX #1: Reuse existing V74 "Update Conversation State" node ✅
- V78.1 FIX #2: Embed State Machine JavaScript directly in workflow JSON ✅
- V78.1 PROBLEM: Switch fallback connected to only 1 node (broke parallel saves)
- V78.1.1 FIX: Switch fallback connects to ALL 5 parallel nodes ✅

ARCHITECTURE V78.1.1 (FULLY CORRECTED):
...
KEY DIFFERENCES FROM V78 COMPLETE:
- ✅ No duplicate "Update Conversation State" node
- ✅ Reuses existing V74 node at position [656, 560]
- ✅ State Machine JavaScript embedded in workflow
- ✅ No manual code copy needed

Date: 2026-04-13
Author: E2 Bot Development Team - V78.1 Final Fix
Version: V78.1 FINAL
"""
...
OUTPUT_V78_1 = Path(__file__).parent.parent / "n8n" / "workflows" / "02_ai_agent_conversation_V78_1_1_FINAL.json"
...
def create_switch_node_v78_1(position, node_id=None):
    """Create properly configured Switch node for V78.1."""
    if node_id is None:
...
def update_state_machine_code(workflow, state_machine_js_path):
    """
    Update State Machine Logic node with V78.1 JavaScript code.
...
def setup_v78_1_connections(workflow, switch_node, http_node1, http_node2, existing_update_conv_node):
    """
    Setup V78.1 connection architecture using EXISTING Update Conversation State node.

    This is the KEY FIX from V78 COMPLETE:
    - V78 COMPLETE: Created new "Update Conversation State" node (duplicate!)
    - V78.1 FINAL: Reuses existing V74 "Update Conversation State" node
    """
    print("\n🔗 Setting up V78.1 connection architecture...")
    ...
    # Keep V74's first connection (Update Conversation State) at index 0
    # Add Switch as SECOND connection at index 1
    # This preserves V74 behavior while adding new routing

    # Actually, let's REPLACE the connection to route through Switch
    ...
    print("   ✅ V78.1 connection architecture complete!")


def generate_v78_1_final():
    """Generate V78.1 FINAL workflow with all fixes."""

    print("=" * 80)
    print("GENERATE WF02 V78.1 FINAL - DUPLICATE NODE FIX + EMBEDDED STATE MACHINE")
    print("=" * 80)
    ...
    workflow["name"] = "02_ai_agent_conversation_V78_1_1_FINAL"
    ...
    switch_node = create_switch_node_v78_1(
    ...
    # Setup V78.1 connections
    setup_v78_1_connections(workflow, switch_node, http_node1, http_node2, existing_update_conv)

    # Save V78.1 FINAL
    print(f"\n💾 Saving V78.1 FINAL to: {OUTPUT_V78_1}")
    OUTPUT_V78_1.parent.mkdir(parents=True, exist_ok=True)

    with open(OUTPUT_V78_1, "w", encoding="utf-8") as f:
        json.dump(workflow, f, indent=2, ensure_ascii=False)

    # Summary
    print("\n" + "=" * 80)
    print("✅ V78.1 FINAL WORKFLOW GENERATED SUCCESSFULLY!")
    print("=" * 80)
    ...
    print(f"\n🔧 Architecture:")
    print("   State Machine Logic (V78.1 code embedded)")
    print("     ↓")
    ...
    print(f"\n📝 Next Steps:")
    print("   1. Import 02_ai_agent_conversation_V78_1_FINAL.json to n8n")
    print("   2. ✅ State Machine code already embedded - no manual copy needed!")
    print("   3. Verify Switch Node has 3 outputs in n8n UI")
    print("   4. Verify WF06 is active:")
    print("      curl http://localhost:5678/webhook/calendar-availability")
    print("   5. Test E2E flow")
    print("   6. Activate V78.1 workflow")

    print(f"\n🆚 V78.1 vs V78 COMPLETE:")
    print("   - V78 COMPLETE: Created duplicate 'Update Conversation State' node ❌")
    print("   - V78 COMPLETE: Required manual State Machine code copy ❌")
    print("   - V78.1 FINAL: Reuses existing V74 node ✅")
    print("   - V78.1 FINAL: State Machine code embedded ✅")
    print("   - V78.1 FINAL: Clean structure, no duplicates ✅")

    return 0


if __name__ == "__main__":
    sys.exit(generate_v78_1_final())
