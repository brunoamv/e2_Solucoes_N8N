// ===================================================
// V100 STATE MACHINE - COLLECTED_DATA FIELD NAME FIX
// ===================================================
...
// V100: Enhanced logging for debugging
console.log('=== V100 STATE MACHINE START (DATA PRESERVATION FIX) ===');
console.log('V100: conversation_id:', input.conversation_id || input.id || 'NULL');
console.log('V100: Current stage:', currentStage);
console.log('V100: Is intermediate state:', isIntermediateState);
console.log('V100: User message:', message);
console.log('V100: Merged currentData keys:', Object.keys(currentData));
console.log('V100: Preserved lead_name:', currentData.lead_name);
console.log('V100: Preserved email:', currentData.email);
console.log('V100: Preserved phone_number:', currentData.phone_number);
console.log('V100: Preserved service_type:', currentData.service_type);
console.log('V100: Preserved service_selected:', currentData.service_selected);
console.log('V100: Input keys:', Object.keys(input));
...
console.log('V100: Has WF06 response:', hasWF06Response);
if (hasWF06NextDates) console.log('V100: Has next_dates data');
if (hasWF06Slots) console.log('V100: Has available_slots data');
...
if (hasWF06Response) {
  if (hasWF06NextDates && currentStage !== 'show_available_dates') {
    console.log('V100: AUTO-CORRECTING state to show_available_dates (has WF06 next_dates)');
    ...
    console.log('V100: AUTO-CORRECTING state to show_available_slots (has WF06 available_slots)');
...
if (isIntermediateState && !message && !hasWF06Response) {
  console.log('V100: Intermediate state without data - maintaining transition');
...
switch (currentStage) {
  ...
  case 'greeting':
  case 'menu':
    console.log('V100: Processing GREETING state');
...
_Digite o número do serviço (1-5):_`;
  ...
  case 'identificando_servico':
    console.log('V100: Processing SERVICE_SELECTION state');

    if (/^[1-5]$/.test(message)) {
      console.log('V100: Valid service number:', message);
      updateData.service_selected = message;
      updateData.service_type = serviceMapping[message];
      console.log('V100: Service mapped:', message, '→', updateData.service_type);
...
_Usaremos para personalizar seu atendimento_`;
      ...
      console.log('V100: Invalid service selection');
...
💡 _Digite 1 ou 2_`;
      ...
      console.log('V100: Skip collect_phone, go directly to WhatsApp confirmation');

    } else {
      console.log('V100: Invalid name format');
...
_Não use apenas números ou caracteres especiais._`;
  ...
  case 'collect_phone_whatsapp_confirmation':
    console.log('V100: Processing PHONE_WHATSAPP_CONFIRMATION state');

    if (message === '1') {
      console.log('V100: WhatsApp number confirmed');
...
_O e-mail nos ajuda a enviar documentação técnica e propostas_`;
      ...
      console.log('V100: User wants alternative phone');
...
_Usaremos este número para agendarmos sua visita técnica_`;
      ...
      console.log('V100: Invalid WhatsApp confirmation option');
      const whatsappPhone = input.phone_number || input.phone_with_code || '';
      responseText = `❌ *Opção inválida*
...
💡 _Digite 1 ou 2_`}`;
  ...
  case 'collect_phone_alternative':
    console.log('V100: Processing COLLECT_PHONE_ALTERNATIVE state');

    const phoneRegex = /^\(?\d{2}\)?\s?9?\d{4}[-\s]?\d{4}$/;

    if (phoneRegex.test(message)) {
      console.log('V100: Valid alternative phone received');
...
_O e-mail nos ajuda a enviar documentação técnica e propostas_`;
      ...
      console.log('V100: Invalid alternative phone format');
...
_Use apenas números, espaços, parênteses e hífen_`;
  ...
  case 'coletando_email':
    console.log('V100: Processing COLLECT_EMAIL state');

    if (message === 'pular') {
      console.log('V100: User skipped email');
...
_Precisamos saber para agendar a visita técnica com nossa equipe local_`;
      ...
      if (emailRegex.test(message)) {
        console.log('V100: Valid email received');
...
_Precisamos saber para agendar a visita técnica com nossa equipe local_`;
        ...
        console.log('V100: Invalid email format');
...
Ou digite *pular* se não quiser informar.`;
  ...
  case 'coletando_cidade':
    console.log('V100: Processing COLLECT_CITY state');

    if (message.length >= 2) {
      console.log('V100: Valid city received');
      ...
      console.log('V100: Confirmation summary data:');
      console.log('V100:   lead_name:', leadName);
      console.log('V100:   phoneNumber:', phoneNumber);
      console.log('V100:   email:', email);
      console.log('V100:   city:', city);
      console.log('V100:   serviceType:', serviceType);
...
3️⃣ *Meus dados estão errados, quero corrigir*`;
      ...
      console.log('V100: Going to State 8 (confirmation)');

    } else {
      console.log('V100: Invalid city (too short)');
...
_Exemplo: Goiânia, Brasília, Anápolis..._`;
  ...
  case 'confirmation':
    console.log('V100: Processing CONFIRMATION state');
    ...
    if (message === '1') {
      ...
      console.log('V100: Service resolution chain:');
      console.log('V100:   service_selected from currentData:', currentData.service_selected);
      console.log('V100:   service_type from currentData:', currentData.service_type);
      console.log('V100:   Final serviceSelected:', serviceSelected);
      console.log('V100:   Final serviceMapped:', serviceMapped);

      if (serviceMapped === 'energia_solar' || serviceMapped === 'projeto_eletrico') {
        console.log('V100: Services 1 or 3 → trigger WF06 next_dates');
        ...
        console.log('V100: Services 2, 4, or 5 → handoff to commercial');
...
3️⃣ *Meus dados estão errados, quero corrigir*`;
  ...
  case 'trigger_wf06_next_dates':
    console.log('V100: INTERMEDIATE STATE - Trigger WF06 next_dates');
    nextStage = 'show_available_dates';
  ...
  case 'show_available_dates':
    console.log('V100: Processing SHOW_AVAILABLE_DATES');
  ...
  case 'process_date_selection':
    console.log('V100: Processing DATE SELECTION');
    ...
    if (/^[1-3]$/.test(dateChoice)) {
      ...
      if (selectedIndex >= 0 && selectedIndex < suggestions.length) {
        ...
        console.log('V100: Date selected:', selectedDate.date);

        updateData.scheduled_date = selectedDate.date;
        updateData.scheduled_date_display = selectedDate.display;

        console.log('V100: Date → trigger WF06 available_slots');
      ...
      console.log('V100: Custom date:', dateChoice);
      ...
      if (isPast) {
        ...
        console.log('V100: Date validated');
  ...
  case 'trigger_wf06_available_slots':
    console.log('V100: INTERMEDIATE STATE - Trigger WF06 available_slots');
    nextStage = 'show_available_slots';
  ...
  case 'show_available_slots':
    console.log('V100: Processing SHOW_AVAILABLE_SLOTS');
  ...
  case 'process_slot_selection':
    console.log('V100: Processing SLOT SELECTION');

    const slotChoice = message.trim();

    if (slotChoice === '0' || slotChoice === 'voltar') {
      console.log('V100: User wants different date');
      ...
      if (selectedIndex >= 0 && selectedIndex < suggestions.length) {
        ...
        console.log('V100: Slot selected:', selectedSlot.formatted);
  ...
  case 'schedule_confirmation':
    console.log('V100: SCHEDULE CONFIRMATION - Flow complete');
...
const output = {
  ...
  version: 'V100',  // V100: Version tracking
  timestamp: new Date().toISOString()
};

// V100: Enhanced logging for state transitions
console.log('V100: Current → Next:', currentStage, '→', nextStage);
console.log('V100: Response length:', responseText.length);
console.log('V100: Update data keys:', Object.keys(output.collected_data));
console.log('V100: Preserved data in collected_data:');
console.log('V100:   lead_name:', output.collected_data.lead_name);
console.log('V100:   email:', output.collected_data.email);
console.log('V100:   phone_number:', output.collected_data.phone_number);
console.log('V100:   service_type:', output.collected_data.service_type);
console.log('V100:   service_selected:', output.collected_data.service_selected);
console.log('V100: Output conversation_id:', output.conversation_id);
console.log('=== V100 STATE MACHINE END ===');
