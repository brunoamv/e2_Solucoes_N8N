// ===================================================
// V101 STATE MACHINE - WF06 SWITCH EXECUTION FIX
// ===================================================
// CRITICAL FIX: Skip switch execution after WF06 auto-correction
// - Track WF06 handling to prevent switch from overwriting state
// - Add root-level fallbacks for Merge node data preservation
...
console.log('=== V101 STATE MACHINE START (DATA PRESERVATION FIX) ===');
console.log('V101: conversation_id:', input.conversation_id || input.id || 'NULL');
console.log('V101: Current stage:', currentStage);
console.log('V101: Is intermediate state:', isIntermediateState);
console.log('V101: User message:', message);
console.log('V101: Merged currentData keys:', Object.keys(currentData));
console.log('V101: Preserved lead_name:', currentData.lead_name);
console.log('V101: Preserved email:', currentData.email);
console.log('V101: Preserved phone_number:', currentData.phone_number);
console.log('V101: Preserved service_type:', currentData.service_type);
console.log('V101: Preserved service_selected:', currentData.service_selected);
console.log('V101: Input keys:', Object.keys(input));
...
console.log('V101: Has WF06 response:', hasWF06Response);
if (hasWF06NextDates) console.log('V101: Has next_dates data');
if (hasWF06Slots) console.log('V101: Has available_slots data');
...
// V101 FIX: Track if WF06 was handled by auto-correction
let wf06HandledByAutoCorrection = false;

// V101 FIX: Auto-correct state if we have WF06 response but wrong state
if (hasWF06Response) {
  if (hasWF06NextDates && currentStage !== 'show_available_dates') {
    console.log('V101: AUTO-CORRECTING state to show_available_dates (has WF06 next_dates)');
    ...
    if (nextDatesResponse.success && nextDatesResponse.dates && nextDatesResponse.dates.length > 0) {
      ...
      // V101 FIX: Mark WF06 as handled by auto-correction
      wf06HandledByAutoCorrection = true;
    }
  } else if (hasWF06Slots && currentStage !== 'show_available_slots') {
    console.log('V101: AUTO-CORRECTING state to show_available_slots (has WF06 available_slots)');
...
// V101 FIX: Skip switch for intermediate states without message and no WF06 response
if (isIntermediateState && !message && !hasWF06Response) {
  console.log('V101: Intermediate state without data - maintaining transition');
...
// V101 FIX: Only process switch if NOT already handled by WF06 auto-correction
else if (!wf06HandledByAutoCorrection && !isIntermediateState) {
  console.log('V101: Entering switch - WF06 not handled by auto-correction');

// ===================================================
// V101 STATE MACHINE LOGIC - ALL STATES IMPLEMENTED
// ===================================================
...
3️⃣ *Meus dados estão errados, quero corrigir*`;
  ...
  case 'process_date_selection':
    ...
    if (/^[1-3]$/.test(dateChoice)) {
      ...
      if (selectedIndex >= 0 && selectedIndex < suggestions.length) {
        ...
        console.log('V101: Date selected:', selectedDate.date);

        updateData.scheduled_date = selectedDate.date;
        updateData.scheduled_date_display = selectedDate.display;

        console.log('V101: Date → trigger WF06 available_slots');
      ...
      console.log('V101: Custom date:', dateChoice);
      ...
      if (isPast) {
        ...
        console.log('V101: Date validated');
  ...
  case 'trigger_wf06_available_slots':
    console.log('V101: INTERMEDIATE STATE - Trigger WF06 available_slots');
    nextStage = 'show_available_slots';
  ...
  case 'show_available_slots':
    console.log('V101: Processing SHOW_AVAILABLE_SLOTS');
  ...
  case 'process_slot_selection':
    console.log('V101: Processing SLOT SELECTION');

    const slotChoice = message.trim();

    if (slotChoice === '0' || slotChoice === 'voltar') {
      console.log('V101: User wants different date');
      ...
      if (selectedIndex >= 0 && selectedIndex < suggestions.length) {
        ...
        console.log('V101: Slot selected:', selectedSlot.formatted);
  ...
  case 'schedule_confirmation':
    console.log('V101: SCHEDULE CONFIRMATION - Flow complete');
  ...
  // V101 FIX: WF06 was handled by auto-correction, skip switch execution
  console.log('V101: Skipping switch - WF06 already handled by auto-correction');
  console.log('V101: Using auto-correction output - nextStage:', nextStage);
  console.log('V101: Using auto-correction output - responseText length:', responseText.length);
}

// ===================================================
// V101 CRITICAL: Build output with data preservation
// ===================================================
const output = {
  ...
  version: 'V101',  // V100: Version tracking
  timestamp: new Date().toISOString()
};

// V100: Enhanced logging for state transitions
console.log('V101: Current → Next:', currentStage, '→', nextStage);
console.log('V101: Response length:', responseText.length);
console.log('V101: Update data keys:', Object.keys(output.collected_data));
console.log('V101: Preserved data in collected_data:');
console.log('V101:   lead_name:', output.collected_data.lead_name);
console.log('V101:   email:', output.collected_data.email);
console.log('V101:   phone_number:', output.collected_data.phone_number);
console.log('V101:   service_type:', output.collected_data.service_type);
console.log('V101:   service_selected:', output.collected_data.service_selected);
console.log('V101: Output conversation_id:', output.conversation_id);
console.log('=== V101 STATE MACHINE END ===');
