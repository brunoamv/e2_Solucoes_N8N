// ===================================================
// V109 STATE MACHINE - FLAG INITIALIZATION FIX
// ===================================================
// CRITICAL BUG FIX: V108 infinite loop caused by undefined flags
//
// ROOT CAUSE (Execution #8989):
// - V108 output construction (lines 938-943) defaults flags to undefined
// - When both updateData and currentData have undefined (not explicitly set)
// - Output has: awaiting_wf06_next_dates = undefined (instead of false)
// - Database stores: NULL (missing from JSONB)
// - V108 detection logic fails: if (awaiting_wf06_next_dates === true && message)
//   → FALSE because undefined !== true
// - User stuck in infinite loop selecting dates
//
// V109 SOLUTION:
// - Flag initialization with explicit false default: (currentData.flag || false)
// - Ensures flags are ALWAYS boolean (true/false), never undefined/null
// - V108 detection logic works correctly: undefined → false → condition FALSE
// - When flags ARE set (true), detection logic works: true → condition TRUE
// - Complete date/slot selection flow without infinite loops
//
// V109 CHANGES:
// 1. Lines 941-946: Add || false to flag initialization
// 2. Version marker: V108 → V109
// 3. Enhanced debug logging: Lines 199, 656
// 4. All log prefixes: V108 → V109
//
// Date: 2026-04-28
// Version: V109 Flag Initialization Fix
// Based on: V108 WF06 Complete Fix
// ===================================================
...
// V108: Enhanced logging for debugging
console.log('=== V108 STATE MACHINE START (WF06 COMPLETE FIX) ===');
console.log('V108: conversation_id:', input.conversation_id || input.id || 'NULL');
console.log('V108: Current stage:', currentStage);
console.log('V108: Is intermediate state:', isIntermediateState);
console.log('V108: Forced stage from awaiting flag:', forcedStage);
console.log('V108: Process WF06 selection immediately:', processWF06Selection);
console.log('V108: User message:', message);
console.log('V108: Merged currentData keys:', Object.keys(currentData));
console.log('V108: Preserved selected_date:', currentData.selected_date);
console.log('V108: Preserved scheduled_date:', currentData.scheduled_date);
console.log('V108: Input keys:', Object.keys(input));
...
console.log('V108: Has WF06 response:', hasWF06Response);
if (hasWF06NextDates) console.log('V108: Has next_dates data');
if (hasWF06Slots) console.log('V108: Has available_slots data');
...
3️⃣ *Meus dados estão errados, quero corrigir*`;
  ...
  case 'process_date_selection':
    ...
    if (/^[1-3]$/.test(dateChoice)) {
      ...
      if (selectedIndex >= 0 && selectedIndex < suggestions.length) {
        ...
        console.log('V108: Date selected:', selectedDate.date);
        ...
        console.log('V108: Date → trigger WF06 available_slots');
      ...
      console.log('V108: Custom date:', dateChoice);
      ...
      if (isPast) {
        ...
        console.log('V108: Date validated');
  ...
  case 'trigger_wf06_available_slots':
    console.log('V108: INTERMEDIATE STATE - Trigger WF06 available_slots');
    console.log('V108: selected_date for WF06:', currentData.selected_date || updateData.selected_date);
    nextStage = 'show_available_slots';
  ...
  case 'show_available_slots':
    console.log('V108: Processing SHOW_AVAILABLE_SLOTS');
  ...
  case 'process_slot_selection':
    console.log('V108: Processing SLOT SELECTION');

    const slotChoice = message.trim();

    if (slotChoice === '0' || slotChoice === 'voltar') {
      console.log('V108: User wants different date');
      ...
      if (selectedIndex >= 0 && selectedIndex < suggestions.length) {
        ...
        console.log('V108: Slot selected:', selectedSlot.formatted);
  ...
  case 'schedule_confirmation':
    console.log('V108: SCHEDULE CONFIRMATION - Flow complete');
...
const output = {
  ...
  version: 'V109',  // V109: Flag initialization fix (default to false instead of undefined)
  timestamp: new Date().toISOString()
};

// V108: Enhanced logging for state transitions
console.log('V108: Current → Next:', currentStage, '→', nextStage);
console.log('V108: Response length:', responseText.length);
console.log('V108: Update data keys:', Object.keys(output.collected_data));
console.log('V108: Preserved data in collected_data:');
console.log('V108:   lead_name:', output.collected_data.lead_name);
console.log('V108:   email:', output.collected_data.email);
console.log('V108:   phone_number:', output.collected_data.phone_number);
console.log('V108:   service_type:', output.collected_data.service_type);
console.log('V108:   service_selected:', output.collected_data.service_selected);
console.log('V108: ✅ CRITICAL - selected_date in collected_data:', output.collected_data.selected_date);
console.log('V108: ✅ CRITICAL - State fields in collected_data:');
console.log('V108:   current_stage:', output.collected_data.current_stage);
console.log('V108:   next_stage:', output.collected_data.next_stage);
console.log('V108:   previous_stage:', output.collected_data.previous_stage);
console.log('V108: ✅ WF06 FLAGS:');
console.log('V108:   awaiting_wf06_next_dates:', output.collected_data.awaiting_wf06_next_dates);
console.log('V108:   awaiting_wf06_available_slots:', output.collected_data.awaiting_wf06_available_slots);
console.log('V108: Output conversation_id:', output.conversation_id);
console.log('=== V108 STATE MACHINE END ===');
