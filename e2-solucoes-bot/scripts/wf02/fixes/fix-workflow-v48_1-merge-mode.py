#!/usr/bin/env python3
"""
V48.1 Fix Script - Merge Node Configuration Fix
Purpose: Fix Merge node mode to properly preserve conversation_id
Root Cause: 'combine' mode doesn't work correctly with multiple inputs
Result: Use 'mergeByPosition' to preserve all fields from all inputs
"""

import json
from pathlib import Path

def fix_workflow_v48_1_merge_mode():
    """Fix Merge node configuration to use mergeByPosition mode"""

    # Paths
    base_dir = Path(__file__).parent.parent
    workflow_v48 = base_dir / "n8n/workflows/02_ai_agent_conversation_V48_CONVERSATION_ID_FIX.json"
    workflow_v48_1 = base_dir / "n8n/workflows/02_ai_agent_conversation_V48_1_MERGE_MODE_FIX.json"

    print("=== V48.1 MERGE MODE FIX ===")
    print(f"Reading: {workflow_v48}")

    # Read workflow V48
    with open(workflow_v48, 'r', encoding='utf-8') as f:
        workflow = json.load(f)

    changes_made = []

    # Fix: Merge Conversation Data - Use mergeByPosition instead of combine
    merge_node = None
    for node in workflow['nodes']:
        if node['name'] == 'Merge Conversation Data':
            merge_node = node
            break

    if merge_node:
        print("\n✅ Found 'Merge Conversation Data' node")
        print(f"   Current mode: {merge_node.get('parameters', {}).get('mode', 'NOT SET')}")

        # Change to mergeByPosition mode which preserves all fields
        merge_node['parameters'] = {
            'mode': 'mergeByPosition',
            'options': {}
        }

        changes_made.append("Changed Merge mode from 'combine' to 'mergeByPosition'")
        print("✅ Changed Merge node to 'mergeByPosition' mode")
        print("   This mode merges items by position and preserves ALL fields")
    else:
        print("❌ ERROR: 'Merge Conversation Data' node not found")

    # Update workflow metadata
    workflow['name'] = "02 - AI Agent Conversation V48.1 (Merge Mode Fix)"
    workflow['versionId'] = "v48-1-merge-mode-fix"

    # Save as V48.1
    print(f"\nSaving fixed workflow: {workflow_v48_1}")
    with open(workflow_v48_1, 'w', encoding='utf-8') as f:
        json.dump(workflow, f, indent=2, ensure_ascii=False)

    print("\n✅ V48.1 Merge Mode Fix Complete!")
    print("\n" + "="*60)
    print("CHANGES MADE:")
    print("="*60)
    for i, change in enumerate(changes_made, 1):
        print(f"{i}. {change}")

    print("\n" + "="*60)
    print("CRITICAL FIX:")
    print("="*60)
    print("✅ Merge node now uses 'mergeByPosition' mode:")
    print("   - Merges items by their position in the array")
    print("   - Preserves ALL fields from ALL inputs")
    print("   - Works correctly with multiple inputs (4 in our case)")
    print("   - conversation_id will be preserved from Get Conversation Details")

    print("\n" + "="*60)
    print("EXPECTED BEHAVIOR:")
    print("="*60)
    print("BEFORE V48.1:")
    print("  ❌ Merge with 'combine' mode → conversation_id lost")
    print("  ❌ State Machine Logic receives NULL conversation_id")
    print("\nAFTER V48.1:")
    print("  ✅ Merge with 'mergeByPosition' mode → all fields preserved")
    print("  ✅ State Machine Logic receives valid conversation_id")
    print("  ✅ UPDATE queries work correctly")
    print("  ✅ State persists in database")
    print("  ✅ Bot progresses correctly (no loop back to menu)")

    print("\n" + "="*60)
    print("TESTING STEPS:")
    print("="*60)
    print("1. Import workflow V48.1 in n8n:")
    print("   http://localhost:5678")
    print("\n2. Deactivate workflow V48")
    print("\n3. Activate workflow V48.1")
    print("\n4. Clean test conversation:")
    print("   docker exec e2bot-postgres-dev psql -U postgres -d e2bot_dev \\")
    print("     -c \"DELETE FROM conversations WHERE phone_number = '556181755748';\"")
    print("\n5. Test with WhatsApp:")
    print("   - Send 'oi' → Bot shows menu ✅")
    print("   - Send '1' → Bot asks for name ✅")
    print("   - Send 'Bruno Rosa' → Bot asks for phone ✅ (NOT menu!)")
    print("\n6. Check logs for V48 diagnostics:")
    print("   docker logs e2bot-n8n-dev 2>&1 | grep 'V48'")
    print("\n   Expected logs:")
    print("   ✅ V48 CONVERSATION ID CHECK")
    print("   ✅ FINAL conversation_id: 8b455c85... (NOT NULL!)")
    print("   ✅ V48: conversation_id validated")
    print("\n7. Verify database:")
    print("   docker exec e2bot-postgres-dev psql -U postgres -d e2bot_dev \\")
    print("     -c \"SELECT phone_number, state_machine_state, contact_name FROM conversations;\"")
    print("\n   Expected:")
    print("   - state_machine_state = 'collect_phone' (NOT 'service_selection')")
    print("   - contact_name = 'Bruno Rosa'")

    print("\n" + "="*60)
    print("VALIDATION:")
    print("="*60)
    print("Check execution doesn't stop after Merge node:")
    print("  - Workflow should continue to State Machine Logic")
    print("  - conversation_id should NOT be NULL")
    print("  - Database should update correctly")
    print("="*60)

    return True

if __name__ == '__main__':
    success = fix_workflow_v48_1_merge_mode()
    exit(0 if success else 1)
